...
#pragma omp for schedule(static)
		...
		* \param save_dist If true, distances are calculated and saved here
		*/
		RECompGP(const den_mat_t& coords_predict,
			...
			has_compact_cov_fct_ = (COMPACT_SUPPORT_COVS_.find(cov_function_->cov_fct_type_) != COMPACT_SUPPORT_COVS_.end()) || apply_tapering_;
			this->num_cov_par_ = cov_function_->num_cov_par_;
			coords_pred_ = coords_predict;
			coords_ = coords;
			if (save_dist) {
				//Calculate distances
				T_mat dist;
				if (has_compact_cov_fct_) {//compactly suported covariance
					CalculateDistancesTapering<T_mat>(coords_, coords_pred_, false, cov_function_->taper_range_, false, dist);
				}
				else {
					CalculateDistances<T_mat>(coords_, coords_pred_, false, dist);
				}
				dist_ = std::make_shared<T_mat>(dist);
				dist_saved_ = true;
			}
		...
		*/
		...
		/*!
		* \brief Subtract the gradient of the predicitive process covariance matrix to get the gradient of the residual covariance matrix in the full scale approximation with tapering
		* \param M1 Matrix sigma_cross_cov_gradient or sigma_cross_cov or sigma_cross_cov * sigma_ip^-1
		* \param M2 Matrix sigma_ip_grad * sigma_cross_cov * sigma_ip^-1 or sigma_ip^-1 * sigma_cross_cov_grad or sigma_ip^-1 * sigma_cross_cov
		*/
		void SubtractGradOfPredProcFromGradOfSigmaForResidInFullScale(const den_mat_t& M1, 
			const den_mat_t& M2, 
			const bool only_triangular) {
			CHECK(sigma_defined_);
			SubtractProdFromMat<T_mat>(sigma_, M1, M2, only_triangular);
		}

		//TODO Tim: DELETE this and replace by SubtractProdFromMat<T_mat>(sigma, M1, M2, only_triangular); in re_model_template.h
		/*!
		* \brief Subtract the gradient of the predicitive process covariance matrix to get the gradient of the residual covariance matrix in the full scale approximation with tapering
		* \param M1 Matrix sigma_cross_cov_gradient or sigma_cross_cov or sigma_cross_cov * sigma_ip^-1
		* \param M2 Matrix sigma_ip_grad * sigma_cross_cov * sigma_ip^-1 or sigma_ip^-1 * sigma_cross_cov_grad or sigma_ip^-1 * sigma_cross_cov
		*/
		void SubtractMatProdOfPredProcForPredResidInFullScale(const den_mat_t& M1, const den_mat_t& M2) {
			CHECK(sigma_defined_);
			SubtractProdFromNonSqMat<T_mat>(sigma_, M1, M2);
		}
		...
		*/
		void ApplyTaper(const T_mat& dist,
			T_mat& sigma) {
			CHECK(dist.rows() == sigma.rows());
			CHECK(dist.cols() == sigma.cols());
			CHECK(apply_tapering_);
		...
		* \param[out] cross_cov Cross-covariance between prediction and observation points / inducing points / prediction points
		* \param[out] uncond_pred_cov Unconditional covariance for prediction points (used only if calc_uncond_pred_cov==true)
		* \param calc_cross_cov If true, the cross-covariance Ztilde*Sigma*Z^T required for the conditional mean is calculated
		* \param calc_uncond_pred_cov If true, the unconditional covariance for prediction points is calculated
		* \param dont_add_but_overwrite If true, the matrix 'cross_cov' is overwritten. Otherwise, the cross-covariance is just added to 'cross_cov'
...
#pragma omp critical
	...
	private:
		...
		/*! \brief Coordinates of prediction points */
		den_mat_t coords_pred_;
