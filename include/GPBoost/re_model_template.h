	...
	public:
		...
		*/
		REModelTemplate(data_size_t num_data,
			...
			num_data_ = num_data;
			//Initialize RNG
			seed_rand_vec_trace_ = seed;
			seed = 10;// delete
			CHECK(seed >= 0);
			Log::REInfo("Seed for sample vec: %i", seed_rand_vec_trace_);
			rng_ = RNG_t(seed);
			//Set up likelihood
			...
			if (num_neighbors_pred == 101) {//Delete
				cg_preconditioner_type_ = "none";
			}
			else {
				cg_preconditioner_type_ = "predictive_process_plus_diagonal";
				//num_rand_vec_trace_ = num_neighbors_pred;
				piv_chol_rank_k_ = num_neighbors_pred;
			}
			if (num_neighbors < 10000) {
				...
				//seed_rand_vec_trace_ = num_neighbors;
				num_rand_vec_trace_ = num_neighbors;
				Log::REInfo("Number of sample vectors for NEGLL and Gradient: %i", num_rand_vec_trace_);
			}
			else {
				piv_chol_rank_ = num_neighbors;
				Log::REInfo("Number of sample vectors for prediction: %i", piv_chol_rank_);
			}
			//Set up GP IDs
			...
			if (std::string(vecchia_pred_type) == "order_obs_first_cond_obs_only") {
				Log::REInfo("No Var Red");
				variance_reduction_ = false;
			}
			else if (std::string(vecchia_pred_type) == "latent_order_obs_first_cond_obs_only") {
				Log::REInfo("Var Red");
				variance_reduction_ = true;
			}
			if (std::string(vecchia_ordering) == "none") {
				Log::REInfo("No Control Variates");
				control_variates_ = false;
			}
			else {
				Log::REInfo("Control Variates");
				control_variates_ = true;
			}
			...
			if (num_gp > 0) {
				...
				if (num_gp_rand_coef_ > 0 && (gp_approx_ == "predictive_process" || gp_approx_ == "full_scale_tapering")) {
					Log::REFatal("Random coefficient GPs are currently not supported for the GP approximation '%s' ", gp_approx_.c_str());
				}
			}
			...
			for (const auto& cluster_i : unique_clusters_) {
				...
				if (gp_approx_ == "predictive_process" || gp_approx_ == "full_scale_tapering") {
			...
			//if (gp_approx_ != "vecchia") {
			if (gp_approx_ != "vecchia" && gp_approx_ != "predictive_process" && gp_approx_ != "full_scale_tapering") { 
		...
		void SetLikelihood(const string_t& likelihood) {
			...
			if (gauss_likelihood_before && !gauss_likelihood_) {
				if (only_one_GP_calculations_on_RE_scale_ || only_one_grouped_RE_calculations_on_RE_scale_) {
					CHECK(gp_approx_ != "predictive_process" && gp_approx_ != "full_scale_tapering");
					for (const auto& cluster_i : unique_clusters_) {
			...
			if (OPTIM_EXTERNAL_.find(optimizer_cov_pars_) != OPTIM_EXTERNAL_.end()) {
				...
				for (int it = 0; it < max_iter_; ++it) {
					...
					if (!na_or_inf_occurred) {
						...
						if ((it < 100 || ((it + 1) % 10 == 0 && (it + 1) < 100) || ((it + 1) % 100 == 0 && (it + 1) < 1000) ||
...
#pragma omp parallel for schedule(static)
			...
			for (const auto& cluster_i : unique_clusters_) {
				if (gp_approx_ == "vecchia") {
					...
					if (gp_approx_ == "predictive_process" || gp_approx_ == "full_scale_tapering") {
						if (matrix_inversion_method_ == "cholesky") {//Cholesky
						...
						else if (matrix_inversion_method_ == "cg") {//Conjugate Gradient
							// Sample probe vectors
							if (!saved_rand_vec_) {//Use same random vectors for each iteration? Delete
								if (!cg_generator_seeded_) {
									cg_generator_ = RNG_t(seed_rand_vec_trace_);
									Log::REInfo("Set Seed: %i", seed_rand_vec_trace_);
									cg_generator_seeded_ = true;
								}
								rand_vec_probe_.resize(num_data_per_cluster_[cluster_i], num_rand_vec_trace_);
								simProbeVect(cg_generator_, rand_vec_probe_, rademacher_);
								saved_rand_vec_ = true;
							}
							std::shared_ptr<den_mat_t> cross_cov = re_comps_cross_cov_[cluster_i][0]->GetZSigmaZt();
							// Transform Rademacher to samples from N(0,P) if Preconditioner
							if (cg_preconditioner_type_ == "predictive_process_plus_diagonal") {
								// Sample Vector with covariance matrix P (Preconditioner)
								if (!saved_rand_vec_id_) {
									rand_vec_probe_id_.resize(num_ind_points_, num_rand_vec_trace_);
									simProbeVect(cg_generator_, rand_vec_probe_id_, rademacher_);
									saved_rand_vec_id_ = true;
								}
								den_mat_t chol_ip_cross_cov_Z = chol_ip_cross_cov_[cluster_i].transpose() * rand_vec_probe_id_;
								rand_vec_probe = chol_ip_cross_cov_Z + diagonal_approx_preconditioner[cluster_i].cwiseSqrt().asDiagonal() * rand_vec_probe_;
							}
							else {
							...
							CGTridiagFSA<T_mat>(*sigma_resid, (*cross_cov), chol_ip_cross_cov_[cluster_i], rand_vec_probe,
							...
							// Variance Reduction (necessary if using preconditioner)
							if (cg_preconditioner_type_ != "none") {
								log_det_Psi_ -= 2. * (((den_mat_t)chol_fact_sigma_ip_[cluster_i].matrixL()).diagonal().array().log().sum());
					...
					}//end gp_approx_ == "predictive_process" or "full_scale_tapering"
		...
		*/
		void EvalNegLogLikelihoodOnlyUpdateFixedEffects(const double* cov_pars,
			double& negll) {
			bool calc_y_aux;//Tim Delete?
			bool calc_y_tilde;//Tim Delete?
			...
			if (only_grouped_REs_use_woodbury_identity_) {
				CalcYtilde(true);//y_tilde = L^-1 * Z^T * y and y_tilde2 = Z * L^-T * L^-1 * Z^T * y, L = chol(Sigma^-1 + Z^T * Z)
				calc_y_tilde = true;//Tim Delete?
			}
			else {
				CalcYAux(1.);//y_aux = Psi^-1 * y
				calc_y_aux = true;//Tim Delete?
			}
			//Calculate quadratic form y^T Psi^-1 y
			CalcYTPsiIInvY(yTPsiInvy_, true, 1, calc_y_aux, calc_y_tilde);//Tim Change?
			negll = yTPsiInvy_ / 2. / cov_pars[0] + log_det_Psi_ / 2. + num_data_ / 2. * (std::log(cov_pars[0]) + std::log(2 * M_PI));
		}
						...
						if (gp_approx_ == "predictive_process" || gp_approx_ == "full_scale_tapering") {
							...
							for (int j = 0; j < num_comps_total_; ++j) {
								...
								if (matrix_inversion_method_ == "cholesky" && gp_approx_ == "full_scale_tapering") {
									...
									CalcLtLGivenSparsityPattern<T_mat>(chol_fact_resid_inv, (*sigma_resid));

									chol_fact_resid_inv.resize(0, 0);
									
								}
								else if (matrix_inversion_method_ == "cg") {
									if (gp_approx_ == "predictive_process") {
										Log::REFatal("The iterative methods are not implemented for Predictive Processes. Please use Cholesky.");
									}
									// P^-1 * sample vectors
									if (cg_preconditioner_type_ == "predictive_process_plus_diagonal") {
										den_mat_t diag_sigma_resid_inv_Z = diagonal_approx_inv_preconditioner[cluster_i].asDiagonal() * rand_vec_probe;
										...
										rand_vec_probe_P_inv = rand_vec_probe;
									}
								}
								for (int ipar = 0; ipar < num_par_comp; ++ipar) {
									// Derivative of Compnents
									std::shared_ptr<den_mat_t> cross_cov_grad = re_comps_cross_cov_[cluster_i][j]->GetZSigmaZtGrad(ipar, true, 1.);
									den_mat_t sigma_ip_stable_grad = *(re_comps_ip_[cluster_i][j]->GetZSigmaZtGrad(ipar, true, 1.));
									...
									if (matrix_inversion_method_ == "cholesky") {
										...
										cov_grad[first_cov_par + ind_par_[j] - 1 + ipar] -= 0.5 * sigma_ip_inv_sigma_ip_stable_grad.trace();
									}
									
									cov_grad[first_cov_par + ind_par_[j] - 1 + ipar] += ((0.5 * sigma_ip_inv_cross_cov_y_aux.dot((sigma_ip_stable_grad)*sigma_ip_inv_cross_cov_y_aux)
									...
									if (gp_approx_ == "full_scale_tapering") {
										...
										if (matrix_inversion_method_ == "cholesky") {
											...
											cov_grad[first_cov_par + ind_par_[j] - 1 + ipar] += 0.5 * ((double)(((*sigma_resid).cwiseProduct(*sigma_resid_grad)).sum()));
										}
											...
											double mean_diagonal_approx_grad_preconditioner;
											if (cg_preconditioner_type_ == "predictive_process_plus_diagonal") {
											...
											den_mat_t sigma_resid_grad_Z = (*sigma_resid_grad) * rand_vec_probe_P_inv;
											...
											if (cg_preconditioner_type_ == "predictive_process_plus_diagonal" && variance_reduction_) {
												P_G_Z = sigma_grad_Z + (diagonal_approx_grad_preconditioner.asDiagonal()) * rand_vec_probe_P_inv - sigma_resid_grad_Z;
											}

											// Stochastic Trace
											stochastic_tr_ = 0;//Tim TODO make not global
											StochTraceFSA<T_mat>(sigma_grad_Z, solution_for_trace_[cluster_i], P_G_Z, rand_vec_probe_P_inv,
												num_data_per_cluster_[cluster_i], stochastic_tr_, sigma_ip_inv_sigma_ip_stable_grad, *sigma_resid_grad, chol_fact_woodbury_preconditioner[cluster_i],
												sigma_woodbury_preconditioner_grad,diagonal_approx_inv_preconditioner[cluster_i],control_variates_,cg_preconditioner_type_, variance_reduction_);
											cov_grad[first_cov_par + ind_par_[j] - 1 + ipar] += 0.5* stochastic_tr_;
										}
										cov_grad[first_cov_par + ind_par_[j] - 1 + ipar] -= 0.5 * y_aux_[cluster_i].dot((*sigma_resid_grad) * y_aux_[cluster_i]) / cov_pars[0];
									}
									else {
										...
										if (ipar == 0) {
											FITC_Diag_grad = FITC_Diag_grad.array() + sigma_ip_stable.coeffRef(0, 0);
										}
										else {
											FITC_Diag_grad = vec_t::Zero(num_data_per_cluster_[cluster_i]);
...
#pragma omp parallel for schedule(static)
										...
										cov_grad[first_cov_par + ind_par_[j] - 1 + ipar] -= 0.5 * y_aux_[cluster_i].dot(FITC_Diag_grad.asDiagonal() * y_aux_[cluster_i]) / cov_pars[0];
										cov_grad[first_cov_par + ind_par_[j] - 1 + ipar] += 0.5 * FITC_Diag_grad.dot(FITC_Diag_[cluster_i].cwiseInverse());
										den_mat_t cross_cov_grad_FITC_inv = (*cross_cov_grad).transpose() * FITC_Diag_[cluster_i].cwiseInverse().asDiagonal();
										cross_cov_grad_sigma_resid_inv_cross_cov_T = cross_cov_grad_FITC_inv * (*cross_cov);
										sigma_woodbury_grad = cross_cov_grad_sigma_resid_inv_cross_cov_T + cross_cov_grad_sigma_resid_inv_cross_cov_T.transpose();
										sigma_woodbury_grad -= (cross_cov_grad_FITC_inv * FITC_Diag_grad.asDiagonal()) * cross_cov_grad_FITC_inv.transpose();
									...
									if (matrix_inversion_method_ == "cholesky") {
										...
										cov_grad[first_cov_par + ind_par_[j] - 1 + ipar] += 0.5 * ((sigma_woodbury_inv_sigma_woodbury_grad.trace()));
									}
								}
							}
						}
						...
						}//end not (GP_approx_ == "predictive_process" || GP_approx_ == "full_scale_tapering")
...
#pragma omp parallel for schedule(static)
					...
					if (gp_approx_ == "vecchia") {//Vechia approximation
						...
						if (gp_approx_ == "predictive_process" || gp_approx_ == "full_scale_tapering") {
							Log::REFatal("Approximation '%s' is currently not implemented for non-Gaussian likelihoods ", gp_approx_.c_str());
						}
						if (only_grouped_REs_use_woodbury_identity_ && !only_one_grouped_RE_calculations_on_RE_scale_) {
...
#pragma omp parallel for schedule(static)
						...
						if (gp_approx_ == "predictive_process" || gp_approx_ == "full_scale_tapering") {
							CalcPredPPFSA(cluster_i, num_data_per_cluster_pred, num_data_per_cluster_,
								...
								predict_var, mean_pred_id, cov_mat_pred_id, var_pred_id, piv_chol_rank_, cg_delta_conv_pred);
						}
						else {
...
#pragma omp parallel for schedule(static)
		...
		*/
		void PredictTrainingDataRandomEffects(const double* cov_pars_pred,
			...
			//Some checks
			if (gp_approx_ != "none") {
				Log::REFatal("PredictTrainingDataRandomEffects() is not yet implemented for the approximation '%s' ", gp_approx_.c_str());
			}
			CHECK(cov_pars_pred != nullptr);
			if (has_covariates_) {
				CHECK(coef_pred != nullptr);
...
#pragma omp parallel for schedule(static) reduction(+:var)
			...
			if (gp_approx_ == "vecchia") {//Neither distances nor coordinates are saved for random coefficient GPs in the Vecchia approximation -> cannot find initial parameters -> just copy the ones from the intercept GP
				...
				for (int j = 0; j < num_comps_total_; ++j) {
					...
					vec_t pars = vec_t(num_par_j);
					if (gp_approx_ == "predictive_process" || gp_approx_ == "full_scale_tapering") {
						re_comps_ip_[unique_clusters_[0]][j]->FindInitCovPar(rng_, pars);
					}
					else {
...
#pragma omp parallel for schedule(static)
	...
	private:
		...
		const std::set<string_t> SUPPORTED_GP_APPROX_{ "none", "vecchia", "tapering", "predictive_process", "full_scale_tapering" };
		...
		int MAX_NUMBER_LR_SHRINKAGE_STEPS_ = 5;
		...
		double cg_delta_conv_pred_ = 0.001;
		...
		/*! \brief if true variance reduction is applied */
		bool variance_reduction_ = true;
		/*! \brief if true Rademacher samples else normal samples */
		bool rademacher_ = true;
		...
		/*! Random number generator used generate Rademacher-vectors in simProbeVect()*/
		RNG_t cg_generator_;
		...
		den_mat_t rand_vec_probe_id_;
		bool saved_rand_vec_ = false;
		bool saved_rand_vec_id_ = false;
		...
		*/
		...
		string_t method_choose_ind_points_ = "random";
...
#pragma omp parallel for schedule(static)
		...
		*/
		void CalcXTPsiInvX(const den_mat_t& X, den_mat_t& XT_psi_inv_X) {
			if (num_clusters_ == 1 && (gp_approx_ != "vecchia" || vecchia_ordering_ == "none") && gp_approx_ != "full_scale_tapering" && gp_approx_ != "predictive_process") {//only one cluster / idependent GP realization
				if (gp_approx_ == "vecchia") {
				...
				for (const auto& cluster_i : unique_clusters_) {
					...
					else if (gp_approx_ == "full_scale_tapering" || gp_approx_ == "predictive_process") {
						std::shared_ptr<den_mat_t> cross_cov = re_comps_cross_cov_[cluster_i][0]->GetZSigmaZt();
						if (matrix_inversion_method_ == "cholesky") {
							if (gp_approx_ == "predictive_process") {
							...
							psi_inv_X.setZero();
							CGFSAMULTI<T_mat>(*sigma_resid, (*cross_cov), chol_fact_sigma_ip_[cluster_i], X, psi_inv_X,
		...
		*/
		void DetermineSpecialCasesModelsEstimationPrediction() {
			...
			only_one_GP_calculations_on_RE_scale_ = num_gp_total_ == 1 && num_comps_total_ == 1 && !gauss_likelihood_ && gp_approx_ != "vecchia" && gp_approx_ != "predictive_process" && gp_approx_ != "full_scale_tapering";//If there is only one GP, we do calculations on the b-scale instead of Zb-scale (currently only for non-Gaussian data)
		...
		*/
		void InitializeIdentityMatricesForGaussianData() {
			if (gauss_likelihood_ && gp_approx_ != "vecchia" && gp_approx_ != "predictive_process" && gp_approx_ != "full_scale_tapering") {
				for (const auto& cluster_i : unique_clusters_) {
		...
		*/
		void CheckCompatibilitySpecialOptions() {
			...
			if (only_one_grouped_RE_calculations_on_RE_scale_) {//only_one_grouped_RE_calculations_on_RE_scale_
				...
				CHECK(gp_approx_ != "vecchia" && gp_approx_ != "predictive_process" && gp_approx_ != "full_scale_tapering");
...
#pragma omp for schedule(static)
		...
		*/
		void CreateREComponentsPPFSA(data_size_t num_data,
			...
			//Log::REInfo("Method to choose inducing points is: '%s'", method_choose_ind_points_.c_str());
			if (num_ind_points_<50) {
				Log::REInfo("CoverTree");
				CoverTree(gp_coords_all_mat, 1/((double)num_ind_points_), rng_, gp_coords_ip_mat);
				num_ind_points_ = gp_coords_ip_mat.rows();
			} else if (num_ind_points_ % 2 == 0) {
				Log::REInfo("random");
				...
				Log::REFatal("Method '%s' is not supported for finding inducing points ", method_choose_ind_points_.c_str());
			}
			
			gp_coords_ip_mat_ = gp_coords_ip_mat;

			/*for (int ii = 0; ii < gp_coords_ip_mat_.rows(); ++ii) {
				for (int jj = 0; jj < gp_coords_ip_mat_.cols(); ++jj) {
					Log::REInfo("gp_coord %d, %d:  %g", ii,jj, gp_coords_ip_mat_.coeffRef(ii,jj));
				}
			}*/
			...
			//Random slope GPs
			if (num_gp_rand_coef_ > 0) {
				Log::REFatal("Random coefficients are currently not supported for '%s' approximation ", method_choose_ind_points_.c_str());
			}
		...
		void CalcSigmaComps() {
			for (const auto& cluster_i : unique_clusters_) {
				for (int j = 0; j < num_comps_total_; ++j) {
					...
					}//end gp_approx_ == "predictive_process" || gp_approx_ == "full_scale_tapering"
					else {
...
//			}
		...
		* \param[out] pred_cov Predictive covariance matrix (only for Gaussian likelihoods)
		* \param[out] pred_var Predictive variances (only for Gaussian likelihoods)
		* \param piv_chol_rank Rank of the pivoted cholseky decomposition used for the preconditioner of the conjugate gradient algorithm
		* \param cg_delta_conv_pred Tolerance level for L2 norm of residuals for checking convergence in conjugate gradient algorithm when being used for prediction
		*/
		void CalcPredPPFSA(data_size_t cluster_i,
			...
			for (int j = 0; j < num_comps_total_; ++j) {
				...
				if (gp_approx_ == "full_scale_tapering") {
					...
					// Cm_inv * C_m,ob
					den_mat_t sigma_ip_inv_cross_cov_ip_ob;
					sigma_ip_inv_cross_cov_ip_ob = chol_fact_sigma_ip_[cluster_i].solve((*cross_cov).transpose());
				...
				if (calc_pred_cov || calc_pred_var) {
					...
					if (piv_chol_rank < 2) {
						//Tsubdiag_k would have size 0 -> Lanczos would crash.
						piv_chol_rank = 2;
					}
					...
					if (gp_approx_ == "full_scale_tapering") {
						...
						std::shared_ptr<T_mat> sigma_resid = re_comps_resid_[cluster_i][j]->GetZSigmaZt();
						if (matrix_inversion_method_ == "cg") {
							if (piv_chol_rank_k_ == 2) {
								Log::REInfo("Approximation with Woodburry and Lanczos only on Residual & CG solve");
								// Lanczos 
								Eigen::SelfAdjointEigenSolver<den_mat_t> ES;
								vec_t b_init = sigma_resid_pred_obs.transpose() * vec_t::Ones(num_data_pred_cli);
								b_init /= num_data_cli;
								vec_t Tdiag_k(piv_chol_rank), Tsubdiag_k(piv_chol_rank - 1), L;
								den_mat_t Q_k(num_data_cli, piv_chol_rank), U;
								LanczosTridiagResid<T_mat>((*sigma_resid), b_init, num_data_cli,
									Tdiag_k, Tsubdiag_k, Q_k, piv_chol_rank, cg_delta_conv_pred);
								ES.computeFromTridiagonal(Tdiag_k, Tsubdiag_k);
								L = ES.eigenvalues();
								U = ES.eigenvectors();
								Log::REInfo("Smallest Eigenvalue: %g", L(0));
								Log::REInfo("biggest Eigenvalue: %g", L(L.size() - 1));
								R = Q_k * U * (L.cwiseSqrt().cwiseInverse()).asDiagonal();
								Log::REInfo("Smallest Eigenvalue: %g", L(0));
								Log::REInfo("biggest Eigenvalue: %g", L(L.size() - 1));
							}
							else if (piv_chol_rank_k_ == 6) {
								Log::REInfo("Approximation with Woodburry & CG solve & Stochastic Diagonal");
								// Sample vectors
								cg_generator_ = RNG_t(seed_rand_vec_trace_);
								den_mat_t rand_vec_probe(num_data_pred_cli, piv_chol_rank);
								simProbeVect(cg_generator_, rand_vec_probe, true);
								den_mat_t rand_vec_probe_pred(num_data_cli, piv_chol_rank);
...
#pragma omp parallel for schedule(static)   
								...
								CGFSA_VARM<T_mat>(*sigma_resid, rand_vec_probe_pred, sigma_resid_inv_pv, NaN_found, num_data_cli, rand_vec_probe_pred.cols(),
...
#pragma omp parallel for schedule(static)   
								...
								vec_t stoch_diag(num_data_pred_cli);
								stoch_diag.setZero();
								if (calc_pred_cov) {
									T_mat M_rand_vec_probe_final_T_mat;
									ConvertTo_T_mat_FromDense<T_mat>((rand_vec_probe_final * rand_vec_probe.transpose()) / piv_chol_rank, M_rand_vec_probe_final_T_mat);
									pred_cov -= M_rand_vec_probe_final_T_mat;
								}
								if (calc_pred_var) {
									if (variance_reduction_) {
										Log::REInfo("Variance Reduction");
										// Control variate
										vec_t diag_var_red(num_data_pred_cli);
										// Stochastic estimation of control variate
...
#pragma omp parallel for schedule(static)   
										for (int i = 0; i < num_data_pred_cli; ++i) {
											diag_var_red[i] = ((vec_t)(R_ptr_2->row(i))).array().square().sum();
										}
...
#pragma omp parallel for schedule(static)   
										...
										// Optimal c
										vec_t c;
										if (control_variates_) {
											Log::REInfo("Optimal c");
											den_mat_t monte_carlo_samples = rand_vec_probe_final.cwiseProduct(rand_vec_probe);
											vec_t monte_carlo_estimate = (vec_t)(monte_carlo_samples.rowwise().sum() / rand_vec_probe.cols());
											den_mat_t monte_carlo_samples_minus_estimate = (den_mat_t)(monte_carlo_samples.colwise() - monte_carlo_estimate);
											den_mat_t monte_carlo_cv = (den_mat_t)((rand_vec_probe_cv.cwiseProduct(rand_vec_probe)).colwise() - diag_var_red);
											vec_t covariance_cv = (vec_t)(monte_carlo_samples_minus_estimate.cwiseProduct(monte_carlo_cv)).rowwise().sum();
											vec_t variance_cv = (vec_t)(monte_carlo_cv.cwiseProduct(monte_carlo_cv)).rowwise().sum();
											c = covariance_cv.cwiseQuotient(variance_cv);

#pragma omp parallel for schedule(static)   
											for (int i = 0; i < c.size(); ++i) {
												if (variance_cv.coeffRef(i) == 0) {
													c[i] = 1;
												}
											}
										}
										...
										Log::REInfo("mean of c: %g", c.mean());
										Log::REInfo("min of c: %g", c.minCoeff());
										Log::REInfo("max of c: %g", c.maxCoeff());
										Log::REInfo("var of c: %g", (c.array() - c.mean()).square().sum() / (c.size() - 1));
										rand_vec_probe_final = rand_vec_probe_final - (den_mat_t)(rand_vec_probe_cv.array().colwise() * c.array());
										stoch_diag += diag_var_red.cwiseProduct(c);
									}
									stoch_diag += ((vec_t)((rand_vec_probe_final.cwiseProduct(rand_vec_probe)).rowwise().sum())) / rand_vec_probe.cols();
									pred_var -= stoch_diag;
								}
							}
						}
...
#pragma omp parallel for schedule(static)
							...
							// Predicitve covariance
							if (calc_pred_cov) {
								T_mat M_sigma_ip_T_mat;
								T_mat chol_sigma_resid_inv_sigma_resid_pred;
								TriangularSolveGivenCholesky<T_chol, T_mat, T_mat, T_mat>(chol_fact_resid_[cluster_i], sigma_resid_pred_obs.transpose(), chol_sigma_resid_inv_sigma_resid_pred, false);
								den_mat_t M_sigma_ip = sigma_resid_pred_obs_sigma_resid_inv_cross_cov * sigma_ip_inv_cross_cov_pred + sigma_ip_inv_cross_cov_pred.transpose() * sigma_resid_pred_obs_sigma_resid_inv_cross_cov.transpose();
								ConvertTo_T_mat_FromDense<T_mat>(M_sigma_ip, M_sigma_ip_T_mat);
								T_mat M_sigma_auto_cov_T_mat;
								ConvertTo_T_mat_FromDense<T_mat>(auto_cross_cov* sigma_ip_inv_cross_cov_pred.transpose(), M_sigma_auto_cov_T_mat);
								T_mat M_sigma_resid_T_mat;
								ConvertTo_T_mat_FromDense<T_mat>(chol_sigma_resid_inv_sigma_resid_pred.transpose()* chol_sigma_resid_inv_sigma_resid_pred, M_sigma_resid_T_mat);
								pred_cov -= M_sigma_ip_T_mat + M_sigma_auto_cov_T_mat + M_sigma_resid_T_mat;
							}
...
#pragma omp parallel for schedule(static)
							...
							if (calc_pred_cov) {
								T_mat M_auto_cross_cov_pred_T_mat;
								den_mat_t M_auto_cross_cov_pred = sigma_resid_pred_obs_Woodburry_fact * auto_cross_cov_pred;
								ConvertTo_T_mat_FromDense<T_mat>(M_auto_cross_cov_pred + M_auto_cross_cov_pred.transpose(), M_auto_cross_cov_pred_T_mat);
								T_mat M_auto_auto_T_mat;
								ConvertTo_T_mat_FromDense<T_mat>(auto_cross_cov_pred* auto_cross_cov_pred.transpose(), M_auto_auto_T_mat);
								T_mat M_Woodburry_T_mat;
								ConvertTo_T_mat_FromDense<T_mat>(sigma_resid_pred_obs_Woodburry_fact* sigma_resid_pred_obs_Woodburry_fact.transpose(), M_Woodburry_T_mat);
								pred_cov -= M_auto_cross_cov_pred_T_mat + M_auto_auto_T_mat + M_Woodburry_T_mat;
							}
						}
					}
					else if (gp_approx_ == "predictive_process") { // Predictive Process
						den_mat_t sigma_ip_inv_cross_cov_pred = chol_fact_sigma_ip_[cluster_i].solve(cross_cov_pred_ip.transpose());
						den_mat_t Fact_FITC_R = (((*cross_cov).transpose() * FITC_Diag_[cluster_i].cwiseSqrt().cwiseInverse().asDiagonal()) * (*cross_cov)) * sigma_ip_inv_cross_cov_pred;
...
#pragma omp parallel for schedule(static)
						...
						if (calc_pred_cov) {
							den_mat_t sigma_ip_stable = *(re_comps_ip_[cluster_i][j]->GetZSigmaZt());
							vec_t FITC = vec_t::Zero(num_data_pred_cli);
							FITC = FITC.array() + sigma_ip_stable.coeffRef(0,0);
							pred_cov += (FITC - PP_Part.diagonal()).asDiagonal();
							T_mat M_Woodburry_T_mat;
							ConvertTo_T_mat_FromDense<T_mat>(Woodburry_fact.transpose()* Woodburry_fact, M_Woodburry_T_mat);
							T_mat M_FITC_T_mat;
							ConvertTo_T_mat_FromDense<T_mat>(Fact_FITC_R.transpose()* sigma_ip_inv_cross_cov_pred, M_FITC_T_mat);
							pred_cov -= M_Woodburry_T_mat + M_FITC_T_mat;
						}
					}
					if (matrix_inversion_method_ == "cg") {
						std::shared_ptr<T_mat> sigma_resid = re_comps_resid_[cluster_i][j]->GetZSigmaZt();
						if (piv_chol_rank_k_ == 2 || piv_chol_rank_k_ == 6) {
							den_mat_t sigma_pred_obs_R(num_data_pred_cli,R.cols());
							if (piv_chol_rank_k_ == 2) {
#pragma omp parallel for schedule(static)
								for (int i = 0; i < R.cols(); ++i) {
									sigma_pred_obs_R.col(i) = sigma_resid_pred_obs * R.col(i);
								}
								if (calc_pred_var) {
#pragma omp parallel for schedule(static)
									for (int i = 0; i < num_data_pred_cli; ++i) {
										pred_var[i] -= sigma_pred_obs_R.transpose().col(i).array().square().sum();
									}
								}
								if (calc_pred_cov) {
									T_mat M_sigma_pred_obs_R_T_mat;
									ConvertTo_T_mat_FromDense<T_mat>(sigma_pred_obs_R* sigma_pred_obs_R.transpose(), M_sigma_pred_obs_R_T_mat);
									pred_cov -= M_sigma_pred_obs_R_T_mat;
								}
							}
							// CG: sigma_resid^-1 * cross_cov
							den_mat_t sigma_resid_inv_cross_cov(num_data_cli, (*cross_cov).cols());
							CGFSA_VARM<T_mat>(*sigma_resid, *cross_cov, sigma_resid_inv_cross_cov, NaN_found, num_data_cli, (*cross_cov).cols(),
								cg_max_num_it_tridiag_, cg_delta_conv_pred_,
								cg_preconditioner_type_, diagonal_approx_inv_preconditioner[cluster_i]);
							// CG: sigma^-1 * cross_cov
							den_mat_t sigma_inv_cross_cov(num_data_cli, (*cross_cov).cols());
							CGFSAMULTI<T_mat>(*sigma_resid, *cross_cov, chol_fact_sigma_ip_[cluster_i], *cross_cov, sigma_inv_cross_cov, NaN_found,
...
#pragma omp parallel for schedule(static)
							...
							if (calc_pred_cov) {
								T_mat M_auto_cross_cov_sigma_T_mat;
								ConvertTo_T_mat_FromDense<T_mat>(auto_cross_cov_sigma_ip_inv_cross_cov_pred.transpose() * sigma_ip_inv_cross_cov_pred, M_auto_cross_cov_sigma_T_mat);
								T_mat M_sigma_resid_pred_obs_sigma_inv_cross_cov_T_mat;
								ConvertTo_T_mat_FromDense<T_mat>(sigma_ip_inv_cross_cov_pred.transpose()* sigma_resid_pred_obs_sigma_inv_cross_cov.transpose() + sigma_resid_pred_obs_sigma_inv_cross_cov * sigma_ip_inv_cross_cov_pred, M_sigma_resid_pred_obs_sigma_inv_cross_cov_T_mat);
								T_mat M_sigma_resid_pred_obs_WF_T_mat;
								ConvertTo_T_mat_FromDense<T_mat>(sigma_resid_pred_obs_WF * sigma_resid_pred_obs_WF.transpose(), M_sigma_resid_pred_obs_WF_T_mat);
								pred_cov -= M_auto_cross_cov_sigma_T_mat + M_sigma_resid_pred_obs_sigma_inv_cross_cov_T_mat - M_sigma_resid_pred_obs_WF_T_mat;
							}
						}
					}
				}//end calc_pred_cov || calc_pred_var
